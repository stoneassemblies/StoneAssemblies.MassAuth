namespace StoneAssemblies.MassAuth.Tests.Hosting.Services
{
    ...
    public class AuthorizationMessageConsumerFacts
    {
        ...
        public async Task Returns_False_In_Authorized_When_Any_Rule_Returns_Fail_By_Error_Async()
        {
            ...
            try
            {
                var requestClient = harness.CreateRequestClient<AuthorizationRequestMessage<AccountBalanceRequestMessage>>();
                var response = await requestClient.GetResponse<AuthorizationResponseMessage>(new AuthorizationRequestMessage<AccountBalanceRequestMessage>());
                Assert.False(response.Message.IsAuthorized);
            }
            finally
            {
